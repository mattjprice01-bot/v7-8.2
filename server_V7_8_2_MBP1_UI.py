...
app=FastAPI(title="US30 Signal Lab V7.8.2 HTF Zones + Databento MBP-1")
...
def pretrade_alerts(c, r: dict) -> None:
    ...
    if perfect:
        body=(
            ...
            f"YM order flow {r.get('databento_score',0):+.2f} · Session {session.get('name')} / {session.get('risk')}\n"
...
@app.get("/health")
def health(): return {"ok":True,"service":"US30 Signal Lab V7.8.2 HTF Zones + Databento MBP-1","databento":get_databento_snapshot().get("status")}
...
<div class="grid3"><div class="card"><div class="label">Calibrated probability</div><div id="calProb" class="value cyan">LEARNING</div><div id="calMeta" class="muted">Building prediction history</div></div><div class="card"><div class="label">HTF swing zone</div><div id="zoneName" class="value" style="font-size:18px">SCANNING</div><div id="zoneMeta" class="muted">D/W/M + old ATH references</div></div><div class="card"><div class="label">Databento YM Order Flow</div><div id="dbScore" class="value">—</div><div id="dbMeta" class="muted">Waiting for live MBP-1</div></div><div class="card"><div class="label">Market session</div><div id="sessionName" class="value" style="font-size:20px">—</div><div id="sessionMeta" class="muted">Session-aware volatility control</div></div></div>
<div class="main">
