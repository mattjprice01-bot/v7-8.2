...
app=FastAPI(title="US30 Signal Lab V7.9 Intelligent State Engine")
...
def pretrade_alerts(c, r: dict) -> None:
    ...
    if perfect:
        ...
        _notify_once(c,key,"PERFECT_ENTRY",f"US30 V7.9 {prob:.1f}% {direction} ENTRY READY",body,5,360,prob)
    elif near:
        body=(
        ...
        _notify_once(c,key,"SETUP_NEAR",f"US30 V7.9 {direction} SETUP {prob:.1f}%",body,3,360,prob)
...
def update_signal_state(c, r: dict) -> dict:
    ...
    prob=r.get("calibrated_probability"); prob=float(prob) if prob is not None else 0.0
    ...
    if changed and new=="SETUP_FORMING":
        _notify_once(c,f"v79:{symbol}:{direction}:forming","V79_SETUP_FORMING",f"US30 V7.9 {direction} SETUP FORMING",f"Probability {prob:.1f}% · Big move {big}% · YM L2 {flow:+.2f}. Waiting for entry confirmation.",3,30,prob)
    elif changed and new=="ENTRY_READY":
        _notify_once(c,f"v79:{symbol}:{direction}:ready","V79_ENTRY_READY",f"US30 V7.9 {direction} ENTRY READY",f"Probability {prob:.1f}% · Price {r.get('price')} · Entry {r.get('entry_low')}–{r.get('entry_high')} · YM L2 {flow:+.2f}. Timing and context confirmed.",5,60,prob)
    elif changed and old_state in ("SETUP_FORMING","ENTRY_READY") and new=="WATCHING":
        _notify_once(c,f"v79:{symbol}:{old_dir}:invalid","V79_INVALIDATED",f"US30 V7.9 {old_dir} SETUP INVALIDATED",f"State returned to WATCHING · probability {prob:.1f}% · YM L2 {flow:+.2f}.",4,20,prob)
    ...
    if flow_reversal:
        _notify_once(c,f"v79:{symbol}:{direction}:flow-reject","V79_ORDERFLOW_REJECT",f"US30 V7.9 {direction} ORDER FLOW REJECTING",f"YM L2 flipped from {prev_flow:+.2f} to {flow:+.2f}. Re-check entry thesis.",4,20,flow)
    out={"state":new,"direction":direction,"changed":changed,"probability":prob,"previous_state":old_state,"orderflow":flow}
    r["trade_state"]=new; r["state_engine"]=out
    return out
...
@app.get("/health")
def health(): return {"ok":True,"service":"US30 Signal Lab V7.9 Intelligent State Engine","databento":get_databento_snapshot().get("status")}
