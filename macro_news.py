...
def get_news(force: bool = False) -> dict:
    ...
    for attempt in range(3):
        try:
            with httpx.Client(timeout=httpx.Timeout(15.0, connect=8.0), headers=UA, follow_redirects=True) as client:
                print(f"[GDELT] attempt {attempt+1}/3 GET {base}", flush=True)
                r = client.get(base, params=params)
                body_preview = (r.text or "")[:500].replace("\n", " ").replace("\r", " ")
                print(f"[GDELT] attempt {attempt+1} HTTP {r.status_code} content-type={r.headers.get('content-type')} body={body_preview!r}", flush=True)
                ...
                print(f"[GDELT] attempt {attempt+1} JSON OK articles={len(articles)}", flush=True)
            ...
            print(f"[GDELT] attempt {attempt+1} ERROR {type(exc).__name__}: {detail}", flush=True)
            if attempt < 2:
    ...
    print(f"[GDELT] all attempts failed; errors={errors}", flush=True)
