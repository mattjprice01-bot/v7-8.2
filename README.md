...
## V7.9.1 dashboard JavaScript hotfix
Fixed a duplicate `st` declaration inside `load()` that caused the browser to reject the entire dashboard script. This prevented `/api/latest` and the other dashboard API requests from running even though TradingView webhooks were returning HTTP 200. The settings object now uses `cfg`, leaving `st` for trade state. JavaScript syntax audit: PASS.
