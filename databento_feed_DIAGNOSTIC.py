...
def _run() -> None:
    ...
    _log(f"worker starting | sdk={'yes' if db is not None else 'no'} | key_present={'yes' if bool(key) else 'no'}")
    ...
    while not _STOP.is_set():
        ...
        try:
            ...
            try:
                ...
                _log("Live() did not accept reconnect/heartbeat args; retrying with key only")
                client = db.Live(key=key)

            start = datetime.now(timezone.utc) - timedelta(minutes=2)
            _log(f"attempt {attempt}: subscribing from {start.isoformat()}")
            ...
            _log(f"attempt {attempt}: subscribe() accepted")

            client.add_callback(
                record_callback=_record_callback,
                exception_callback=_exception_callback,
            )
            _log(f"attempt {attempt}: callbacks registered; starting live session")

            client.start()
            _log(f"attempt {attempt}: client.start() returned; waiting for records/close")
            client.block_for_close()

            _log(f"attempt {attempt}: live session closed")
        except Exception as exc:
            _exception_callback(exc)
            _log("traceback:\n" + traceback.format_exc())

        if not _STOP.wait(5):
            _log("retrying Databento connection in 5s")
            continue
